...
{
    int i=0,j,k=0,l=0,m=0,ns=0,sys,prn;
    char buff[1024];
    ...
    while (!feof(fp))
	{
        ...
        else if (2<=i&&i<=20&&buff[1]==' ')//读不读不影响，数据体依据每行开头标号载入星历数据 by ymd 2021.4.9
		{
            if (i==2) 
			{
				if (buff[3] != ' ')
		...
		else if (buff[0]=='%'&&buff[1]=='c'&&l==0) 
		{
			l++;
            strncpy(tsys,buff+9,3); tsys[3]='\0';
        }
        else if (buff[0] == '%'&&buff[1] == 'f'&&m == 0) 
		{
			m++;
	...
	char *tsys, int index, int opt, nav_t *nav)
{
	...
	while (!feof(fp)) {
		...
		for (i=pred_o=pred_c=v=0;i<n&&fgets(buff,sizeof(buff),fp);i++) {
			...
			sys=buff[1]==' '?SYS_GPS:code2sys(buff[1]); //若GPS单系统则SP3开头不写系统标识字母
...
extern void readsp3(const char *file, nav_t *nav, int opt)
{
	...
	/* read sp3 header */
	ns=readsp3h(fp,&time,&type,sats,bfact,tsys); //sats not used?
