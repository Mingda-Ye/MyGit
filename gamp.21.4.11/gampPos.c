...
static void procpos(rtk_t *rtk,const prcopt_t *popt,const solopt_t *sopt,int mode)
{
	...
	while ((nobs=inputobs(obs,obss,PPP_Glo.revs,&PPP_Glo.iObsu,&PPP_Glo.iEpoch))>=0) {  //obs数据导入处,by ymd,2021.4.10
	               ...
	               const solopt_t *sopt, filopt_t *fopt)
{
	...
	if (PMODE_PPP_KINEMA<=popt->mode)
		setcodepri(SYS_GPS,1,popt->sateph==EPHOPT_PREC?"PXWC":"CPYW");//setting frequency priority for GPS L2 band from PYWC to PXWC,Modified by ymd, 2021.4.9
