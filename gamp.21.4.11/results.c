...
static int outpos(unsigned char *buff, const char *s, const sol_t *sol, const solopt_t *opt)
{
	...
	double pos[3],dxyz[3],denu[3],base[3],basell[3];
    ...
    base[2]=3379986.9144;
    for(int k=0;k<3;k++)
        dxyz[k]=sol->rr[k]-base[k];
    ecef2pos(base,basell);
    ecef2enu(basell, dxyz, denu);
    ecef2pos(sol->rr,pos);
    
	p += sprintf(p, "%04d%s%02d%s%02d%s%02d%s%02d%s%02d%s%4d%s%9.2f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f%s%14.4f",
		(int)ep[0], sep, (int)ep[1], sep, (int)ep[2], sep, (int)ep[3], sep, (int)ep[4], sep, (int)ep[5], sep, week, sep, sow, sep,
// 		sol->rr[0], sep, sol->rr[1], sep, sol->rr[2], sep, sol->qr[0], sep, sol->qr[1], sep, sol->qr[2]);
		denu[0], sep, denu[1],  sep, denu[2], sep, sol->qr[0], sep, sol->qr[1], sep, sol->qr[2]);
//        pos[0]*180/PI, sep, pos[1]*180/PI, sep, pos[2]*180/PI, sep, sol->qr[0], sep, sol->qr[1], sep, sol->qr[2]);
...
// 	p+=sprintf(p,"\n");
	...
	int prn = 0;
	char sys;
	for (int k = 0; k < sizeof(rtk->ssat) / sizeof(rtk->ssat[0]); k++)
	{
		if (rtk->ssat[k].azel[1] != 0)
		{
			/*if (k<=NSATGPS)
			{
				prn = k;
			...
			else if (k<= NSATGLO + NSATGPS)
			{
				prn = k - NSATGPS;
				sys = 'R';
			}
			else if (k<= NSATGAL + NSATGLO + NSATGPS)
			{
				prn = k - NSATGLO - NSATGPS;
				sys = 'E';
			}
			else if (k<= NSATGAL + NSATGLO + NSATGPS + NSATCMP)
			{
				prn=k- NSATGAL - NSATGLO - NSATGPS;
				sys = 'C';
			}
			else if (k< NSATGAL + NSATGLO + NSATGPS + NSATCMP + NSATQZS)
			{
				prn = k - NSATGAL - NSATGLO - NSATGPS - NSATCMP;
				sys = 'J';
			}*/
			prn = k;
			p += sprintf(p, "%d%s%9.3f%s%9.3f%s\n", prn, sep, rtk->ssat[k].azel[0] * R2D, sep, rtk->ssat[k].azel[1] * R2D, sep);
		}
	}
	n=p-(char *)buff;
