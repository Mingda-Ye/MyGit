	...
	filopt_t *filopt)
{
	...
	int debug=0;//Index of whether output information while debugging. By ymd 2021.4.9
	...
	prcopt->niter     = 2;    //modified by ymd 2021.4.9 (1->2)
	prcopt->maxinno   = 30.0;
	prcopt->dynamics  = 0;
	...
	while (!feof(fp)) {
		...
		if (strstr(line,"start_time")) {//strstr()用于检测str1是否包含str2 /learned by ymd 2021.4.9
