                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    for (i=n=0;*tobs[i];i++,n++)
    {
        ...
        if (sys==SYS_CMP) {     //Notice:Frequency index for BDS, by ymd, 2021.4.9
            if      (ind->frq[i]==4) ind->frq[i]=2; /* B3 */
            else if (ind->frq[i]==5) ind->frq[i]=3; /* B2 */
        }
                       ...
                       char tobs[][MAXOBSTYPE][4], int *flag, obsd_t *data)
{
    ...
    /* set signal index */
    //设定信号标签，决定了采用哪些信号 by ymd, 2021.4.9
